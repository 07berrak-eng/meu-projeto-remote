...
class WebRtcSession(
    ...
    fun start() {
        ...
        Thread {
            try {
                ...
                onStatus("Falha ao iniciar: ${e.message}", false)
            }
        }.start()
    }
    ...
    private fun ligarSocket() {
        ...
        s.on(Socket.EVENT_CONNECT) {
            val hello = JSONObject().put("op", op)
                .put("userAgent", "AtlasAndroid/1.0 (Android ${Build.VERSION.RELEASE}; ${Build.MODEL})")
            sessaoToken?.let { hello.put("token", it) }
            s.emit("cliente:hello", hello)
            onStatus("Ligado. A preparar a partilha…", true)
        }
        s.on("cliente:sessao") { args ->
            try {
                val d = args[0] as JSONObject
                if (d.has("token")) {
                    sessaoToken = d.getString("token")
                    prefs.edit().putString("token_$op", sessaoToken).apply()
                }
                s.emit("cliente:partilhar", JSONObject().put("ativo", true))
        ...
        s.on("cliente:erro") { args ->
            ...
            onStatus("Erro: $msg", false)
        }
        s.on("tecnico:pronto") { criarOferta() }
        s.on("webrtc:answer") { args ->
            try {
                val d = args[0] as JSONObject
                aplicarAnswer(d.getJSONObject("sdp"))
            } catch (e: Exception) { Log.w(TAG, "answer: ${e.message}") }
        }
        s.on("webrtc:ice") { args ->
            try {
                val d = args[0] as JSONObject
                if (d.has("candidate") && !d.isNull("candidate")) {
                    adicionarIce(d.getJSONObject("candidate"))
                }
    ...
    private fun aplicarAnswer(sdpObj: JSONObject) {
    ...
    private fun adicionarIce(candObj: JSONObject) {
